...
CURRENT_SITE_URL = "https://musclelove-777.github.io/entsuma/"
...
{{ actresses }}さんが出演するこの作品。熟女・人妻ファンなら見逃せない一本です。
{% endif %}

{% if maker %}
> **{{ maker }}**からリリースされたこの作品は、熟女・人妻ジャンルで定評があります。
{% endif %}
...
INTRO_VARIATIONS = [
    "色気がたまらない...！**「{title}」**は{genre_text}好きにはたまらない一本です。",
    "熟女・人妻好きにはたまらない...！**「{title}」**、{genre_text}好きなら絶対ハマる作品が来ました。",
    "{genre_text}で興奮したいならコレ！**「{title}」**がマジでおすすめ。",
    "話題沸騰中の**「{title}」**をピックアップ。{genre_text}のド直球熟女作品です。",
    "**「{title}」**が気になってる人、正解です。{genre_text}ジャンルの中でもガチで抜ける熟女作品。",
    "本日の厳選熟女・人妻は**「{title}」**。{genre_text}好きの間で「神作品」と話題に。",
    "新着から見つけた掘り出し物！**「{title}」**、{genre_text}がたっぷり詰まった艶やかな一本です。",
    "サンプル動画だけでも興奮必至！**「{title}」**は{genre_text}の最高傑作かも。",
    "今週一番シコれる熟女作品はコレ。**「{title}」**、{genre_text}好きは見逃すな！",
    "MuscleLoveが厳選！**「{title}」**は{genre_text}好きなら見逃せない熟女・人妻作品です。",
    "MuscleLove編集部おすすめの**「{title}」**。{genre_text}ジャンルで今一番アツい熟女作品。",
    "MuscleLoveイチオシ！**「{title}」**、{genre_text}好きを唸らせる最高の熟女・人妻作品が来ました。",
]
...
def _generate_single_article(
    ...
    # タグの生成
    tag_list = genres[:5] if genres else ["熟女", "人妻"]
    tags = ", ".join(f'"{t}"' for t in tag_list)

    # ジャンルテキスト（導入文用）
    genre_text = "・".join(genres[:3]) if genres else "熟女"
...
def _build_meta_description(title: str, genre_text: str, actresses: str) -> str:
    ...
    desc_variations = [
        f"{title}のサンプル動画・レビュー。熟女・人妻の{genre_text}系作品を紹介。",
        f"熟女好き必見の「{title}」を徹底レビュー。{genre_text}好きにおすすめの人妻AV。",
        f"色気MAXの{genre_text}作品「{title}」。熟女・人妻シチュエーションに興奮必至。",
        f"熟女作品の注目作「{title}」。{genre_text}の大人の魅力をサンプル動画付きで紹介。",
    ]
    if actresses:
        desc_variations.append(
            f"{actresses}出演「{title}」。{genre_text}系熟女・人妻動画をチェック。"
        )
...
def _build_alt_text(title: str, actresses: str, genre_text: str) -> str:
    ...
    alt_variations = [
        f"熟女・人妻作品「{title}」のパッケージ画像",
        f"「{title}」{genre_text}系熟女作品のサムネイル",
        f"色気がたまらない「{title}」の作品画像",
    ]
    if actresses:
        alt_variations.append(f"{actresses}出演「{title}」熟女・人妻作品の画像")
...
"""
    link_url = affiliate_url or images[0]
    for idx, img_url in enumerate(images, 1):
        gallery_html += f'  <a href="{link_url}" target="_blank" rel="nofollow sponsored"><img src="{img_url}" alt="熟女・人妻作品のサンプル画像{idx}" style="width: 100%; border-radius: 4px;" loading="lazy" /></a>\n'
